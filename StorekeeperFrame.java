	...
	public void actionPerformed(ActionEvent event)
	{
		...
		try
		{
			...
			if (sourceButton == processTransactionButton)
			{
				...
				try
				{
					query = "SELECT * FROM inventory WHERE BINARY id='" + productID + "' AND isDeleted=0;";
						...
						if (saleRadioButton.isSelected())
						{
							if (quantityFromTable >= quantity)
							{
								...
								if (creditsDueFilled)
								{
									...
									query = "SELECT * FROM customer WHERE id='" + posCustomerID + "' AND isDeleted!=1;";
									resultSet = statement.executeQuery(query);
										{
			...
			else if (sourceButton == processCreditsButton)
			{
				...
				try
				{
					query = "SELECT * FROM customer WHERE BINARY id='" + creditCustomerID + "' AND isDeleted=0;";
					resultSet = statement.executeQuery(query);
					if(!resultSet.isBeforeFirst())
					{
			...
			else if (sourceButton == registerDialogButton)
			{
				try
				{
					...
					query = "INSERT INTO customer(id,fname,lname,address,phone,credits_amount,added_in,added_by) VALUES('" + newCustomerID + "','" + customerFname + "','" + customerLname + "','" + customerAddress + "','" + customerPhone + "',"+newCustomerAmount+",'"+dateTimeStamp+"','"+loginID+"');";
					int rowsAffected = statement.executeUpdate(query);
					if (rowsAffected > 0)
					{
				...
				{
					JOptionPane.showMessageDialog(this,"Customer ID is not available!","GroceryDesk- Error!",JOptionPane.ERROR_MESSAGE);
				}
				catch (SQLException e)
				{
					e.printStackTrace();
			...
			else if(sourceButton==inventoryRemoveEntryButton)
			{
				...
				query="SELECT id FROM inventory WHERE isDeleted!=1 LIMIT "+inventorySelectedRowNumber+",1";
				resultSet=statement.executeQuery(query);
				if(resultSet.next())
				{
					idFromInventoryTable=resultSet.getString(1);
				}
				query="UPDATE inventory SET isDeleted=1 WHERE id='"+idFromInventoryTable+"';";
				int rowsAffected=statement.executeUpdate(query);
				if(rowsAffected>0)
				{
			...
			else if(sourceButton==creditAccountsRemoveEntryButton)
			{
				...
				query="SELECT id FROM customer WHERE isDeleted!=1 LIMIT "+creditAccountsSelectedRowNumber+",1";
				resultSet=statement.executeQuery(query);
				if(resultSet.next())
				{
					idFromCreditAccountsTable=resultSet.getString(1);
				}
				query="UPDATE customer SET isDeleted=1 WHERE id='"+idFromCreditAccountsTable+"';";
		...
		{
			JOptionPane.showMessageDialog(this,"Product ID is not available!","GroceryDesk- Error!",JOptionPane.ERROR_MESSAGE);
		}
		catch(Exception e)
		{
			e.printStackTrace();
...
//		adminNav.startSystem(loginID);
	...
	public void showInventoryRowInfo(MouseEvent e)
	{
		...
		try
		{
			String query = "SELECT * FROM inventory WHERE isDeleted!=1 LIMIT "+inventorySelectedRowNumber+",1;";
			resultSet = statement.executeQuery(query);
			if(resultSet.next())
			{
	...
	public void showCreditAccountsRowInfo(MouseEvent e)
	{
		...
		try
		{
			String query = "SELECT * FROM customer WHERE isDeleted!=1 LIMIT "+creditAccountsSelectedRowNumber+",1;";
			resultSet = statement.executeQuery(query);
			if(resultSet.next())
			{
