//TODO
/* Check INSERTION into tables. If isDeleted=1, donot insert! */
/*
Check INSERTION into tables. Because of isDeleted Column being hidden, problems arise.
Refer to Insertion in ViewStorekeeper.java, if necessary.

For insertion in Float or any numeric fields, use If.. else to determine if selectedColumnName(variable name with reference from ViewStorekeeper.java)
is 'Amount' or 'Rate' Column, hardcore.
...
public class StorekeeperFrame extends JFrame implements ActionListener, DocumentListener, ItemListener
{
	...
	static boolean creditsDueFilled=false,customerIDFilled=false,outOfStockError=false;
	...
	{
		//final StorekeeperFrame storekeeperFrame = new StorekeeperFrame("root","");
		//storekeeperFrame.startSystem("demo",0,0);
	}

	public StorekeeperFrame(String databaseID, String databasePassword)
	{
	...
	//Privilege 0 for Storekeepers, 1 for Administrator
	public void startSystem(String loginID,int index,int loginPrivilege)
	{
		...
		logoutMenuButton.addMouseListener(new MouseAdapter()
		{
			...
			{
				super.mouseExited(e);
				logoutMenuButton.setForeground(Color.BLACK);
			}
		});
...
////		});
		...
		aboutProjectDialog.setSize(425,380);
		...
		String aboutText="<div style='color:white; font-family: Segoe UI; text-align:justify'><center><p>GroceryDesk is an attempt at making grocery storekeeping easier for every role involved in the business.</p></center><br>";
		...
		/* Constraints for Components */
		aboutProjectDialogLayout.putConstraint(SpringLayout.NORTH,aboutProjectTextPane,50,SpringLayout.NORTH,aboutProjectDialogContentPane);
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		try
		{
			...
			if (sourceButton == processTransactionButton)
			{
				...
				else if(quantityTextField.getText().length()>=11)
				{
			...
			else if (sourceButton == processCreditsButton)
			{
				...
				if(creditCustomerID.trim().length()<=0)
				{
					JOptionPane.showMessageDialog(this, "Customer ID field should not be left blank!", "GroceryDesk- Error", JOptionPane.ERROR_MESSAGE);
				}
			...
			else if (sourceButton == registerDialogButton)
			{
				try
				{
					...
					if (newCustomerID.trim().length() <= 0 && customerFname.trim().length() <= 0 && customerLname.trim().length() <= 0 && customerAddress.trim().length() <= 0 && customerPhone.trim().length() <= 0)
					{
						JOptionPane.showMessageDialog(this, "The fields cannot be left blank!", "GroceryDesk- Error", JOptionPane.ERROR_MESSAGE);
					} else if (newCustomerID.trim().length() <= 0)
					{
					...
					else if (newCustomerID.length() >25)
					{
					...
					} else if (customerFname.length() >25)
					{
					...
					} else if (customerLname.length() >25)
					{
				...
				{
					JOptionPane.showMessageDialog(this,"The specified Customer ID is not available!","GroceryDesk- Error!",JOptionPane.ERROR_MESSAGE);
				}
				catch (SQLException e)
				{
					e.printStackTrace();
			...
			{
				boolean numberFormatExceptionCatch = false;
				...
				if(newProductID.trim().length() <=0 && newProductName.trim().length() <=0)
				{
...
//				else if(newProductID.equals(""))
//				{
//					JOptionPane.showMessageDialog(this,"The Product ID field cannot be left blank!","GroceryDesk- Error",JOptionPane.ERROR_MESSAGE);
//				}
//				else if(newProductName.equals(""))
//				{
//					JOptionPane.showMessageDialog(this,"The Product Name field cannot be left blank!","GroceryDesk- Error",JOptionPane.ERROR_MESSAGE);
//				}
				else if(newProductID.trim().length() <=0)
				{
					JOptionPane.showMessageDialog(this,"The Product ID field cannot be left blank!","GroceryDesk- Error",JOptionPane.ERROR_MESSAGE);
				}
				...
				else if (!numberFormatExceptionCatch)
				{
		...
		{
			JOptionPane.showMessageDialog(this,"The specified Product ID is not available!","GroceryDesk- Error!",JOptionPane.ERROR_MESSAGE);
		}
		catch(Exception e)
		{
			e.printStackTrace();
