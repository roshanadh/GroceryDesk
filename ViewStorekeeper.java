//TODO Remove
/*
BUG- WHEN AN ADMIN OPENS 'View Storekeepers' FRAME, THE ADMIN CAN LOGOUT BEFORE CLOSING THE 'View Storekeepers' FRAME.
 */
...
public class ViewStorekeeper extends JFrame implements ActionListener
{
	...
	public void startSystem()
	{
		...
		frameLayout.putConstraint(SpringLayout.NORTH,messageLabel,10,SpringLayout.NORTH,frameContentPane);
		frameLayout.putConstraint(SpringLayout.WEST,messageLabel,10,SpringLayout.WEST,frameContentPane);

		frameLayout.putConstraint(SpringLayout.NORTH,storekeeperTableSp,0,SpringLayout.SOUTH,messageLabel);
		frameLayout.putConstraint(SpringLayout.WEST,storekeeperTableSp,0,SpringLayout.WEST,messageLabel);
		frameLayout.putConstraint(SpringLayout.SOUTH,storekeeperTableSp,-10,SpringLayout.SOUTH,frameContentPane);
		frameLayout.putConstraint(SpringLayout.EAST,storekeeperTableSp,-10,SpringLayout.EAST,frameContentPane);
...
//			storekeeperTableModel.setColumnIdentifiers(storekeeperTableColumns);
	...
	public void showStorekeeperRowInfo(MouseEvent e)
	{
		...
		try
		{
			String query = "SELECT * FROM storekeeper WHERE isDeleted!=1 LIMIT "+storekeeperSelectedRowNumber+",1;";
...
//		try
//		{
//			String query = "SELECT added_in,added_by FROM storekeeper WHERE "+selectedColumnName+"='"+selectedCellValue+"';";//LIMIT "+storekeeperSelectedRowNumber+",1;";
//			resultSet = statement.executeQuery(query);
//			if(resultSet.next())
//			{
//				String addedIn = resultSet.getString(1);
//				String addedBy = resultSet.getString(2);
//				storekeeperGetAddedInLabel.setText(addedIn);
//				storekeeperGetAddedByLabel.setText(addedBy);
//			}
//			resultSet.close();
...
//				query = "SELECT id FROM storekeeper WHERE "+selectedColumnName+"='"+selectedCellValue+"';";
//				resultSet = statement.executeQuery(query);
//				if (resultSet.next())
//				{
//					idFromStorekeeperTable = resultSet.getString(1);
//					System.out.println("ID== "+idFromStorekeeperTable);
//				}
//				query = "UPDATE storekeeper SET isDeleted=1 WHERE id='" + idFromStorekeeperTable + "';";
...
//					JOptionPane.showMessageDialog(this, "Record couldn't be removed!", "GroceryDesk- Error!", JOptionPane.ERROR_MESSAGE);
				...
				String idFromStorekeeperTable = null;
				query="SELECT id FROM storekeeper WHERE isDeleted!=1 LIMIT "+storekeeperSelectedRowNumber+",1";
				resultSet=statement.executeQuery(query);
				if(resultSet.next())
				{
					idFromStorekeeperTable=resultSet.getString(1);
